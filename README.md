...
```bash
npm run polices      # télécharge les familles de outils/polices.js → web/polices/
```

Le téléchargement n'appartient pas à la construction, qui doit tourner sans
réseau. On ne le relance que pour changer la liste des familles, en tête de
`outils/polices.js` ; il écrit aussi `outils/polices.json`, que la construction
relit, et une feuille par famille (`web/polices/<famille>.css`) pour une page
qui en charge une à la demande. Une page qui demanderait encore une police à
Google fait échouer `npm run construire`.
