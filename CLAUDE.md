...
| rappel au visiteur qui a l'application et lit le navigateur | `_installation.html` § L'application déjà posée sur l'appareil — `appliInstallee`, démenti du système `verifieApplication`, ouverture `lanceApplication` (`intent://`) ; fenêtres `ouvreRappel` et son repli `ouvreRetrouve`, aiguillées par `essaieInvitation` ; délai `RAPPEL_DELAI` ; `related_applications` du manifeste dans `outils/pwa.js`, adressé par `src/index.mjs` `manifeste` |
