...
| capacité d'un stand, journée organisée qui s'étale | réglage `_admin1.html` `voletParcours` (onglet « Parcours intelligent », `REGLAGES_CAPACITE` — `parDix`, `plancher`, `plafond`, `nombre` —, `capaciteGeree`, `capaciteStand`, surface par `aireDuStand`), clé `_capacite` ; **un prix, jamais un refus** — courbe `_journee.html` `PEINE_CHARGE`, `PEINE_MAX`, `SEUIL_PEINE`, `peineDeCharge`, entrée dans le calcul par `coutCreneau` seul (`peineCreneau`, `peineCandidat`, `peineDecalee`) ; dilatation quand le salon est globalement surdemandé `facteurDeCharge` (`TRANCHES_MINI`, `FACTEUR_MAX`) ; annonce et lecture `_journee.html` § La charge annoncée (`annoncePlan`, `litLaCharge`, `chargeCellule`, `trancheDe`, `plafondMinimal`), identifiant du parcours `_parcours.html` `identifiantParcours` ; serveur `supabase/functions/plan-de-visite/`, relais `src/index.mjs` `planDeVisite` et `chargePrevue` (`/api/plan-de-visite`, `/api/charge`), migration `la_charge_prevue_des_stands` |
