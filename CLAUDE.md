...
| fenêtre qui invite le visiteur à installer le plan | `_installation.html` — moment `essaieInvitation` (lancé par `_admin2.html` `demarre`), façon par navigateur `faconInstallation`, confirmation après installation `ouvreInstalle`, qui ouvre l'application d'un bouton là où `LANCEMENT_POSSIBLE` ; case `caseInstallation`, posée par `_admin1.html` `voletAdmin`. Ne jamais y écrire `rel="manifest"` entre guillemets : la construction y reconnaît la page installable |
