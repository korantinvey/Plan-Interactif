...
| plafond de visiteurs simultanés sur un stand, journée organisée qui s'étale | réglage `_admin1.html` `voletParcours` (onglet « Parcours intelligent », `REGLAGES_CAPACITE`, `capaciteGeree`, `capaciteStand`, surface par `aireDuStand`), clé `_capacite` ; contrainte `_journee.html` `rangeSejour` — `trop` et `sature` entrent dans `deborde`, soupape dans `depart` ; annonce et lecture `_journee.html` § La charge annoncée (`annoncePlan`, `litLaCharge`, `chargeCellule`, `trancheDe`), identifiant du parcours `_parcours.html` `identifiantParcours` ; serveur `supabase/functions/plan-de-visite/`, relais `src/index.mjs` `planDeVisite` et `chargePrevue` (`/api/plan-de-visite`, `/api/charge`), migration `la_charge_prevue_des_stands` |
