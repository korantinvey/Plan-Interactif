...
function indexe(d){
  ...
  $("titre").textContent = DATA.evenement || "Plan du salon";
...
function liste(){
  ...
  box.innerHTML = vus.length ? vus.slice(0, 500).map(o => {
    ...
    return '<button class="row ' + (o.kind === "zone" ? "z" : "") + '" data-id="' + o.id +
      ...
      '<span><span class="nm">' + esc(o.nom || o.plan || ("Stand " + (o.code || ""))) + '</span><br>' +
...
function ouvre(o, rect){
  ...
  $("dName").textContent = o.nom || o.plan || ("Stand " + (o.code || ""));
  $("dCode").textContent = DATA.plans[o.p].libelle + (o.code ? " · " + o.code : "");
  ...
  $("dBody").innerHTML =
    f("Surface", o.m2 ? o.m2 + " m²" : "") +
    f("Angles ouverts", o.angles != null && !zone ? String(o.angles) : "") +
    f("Niveaux", o.niveaux > 1 ? String(o.niveaux) : "") +
    f("Commercialisation", !zone && o.etat ? esc(o.etat) : "") +
    f("Site web", url ? '<a href="' + esc(url) + '" target="_blank" rel="noopener">' +
                        esc(url.replace(/^https?:\/\//, "")) + '</a>' : "") +
    (o.nomencl && o.nomencl.length
      ? '<div class="field"><span class="eyebrow">Nomenclature</span><div class="tags">' +
        o.nomencl.map(n => '<span class="tag">' + esc(String(n).replace(/^SMCL26_/, "")) + '</span>').join("") +
        '</div></div>' : "") +
...
/* ============================================================
   10. Menu d'administration
   Ctrl + Alt + A. Le paramètre d'URL ne traverse pas l'iframe d'une page
   publiée, ce raccourci si — et il reste invisible pour un visiteur.
   ============================================================ */
const CLE_ADMIN = "plan-admin";
let ADMIN = /(?:^|[?&])admin=1(?:&|$)/.test(location.search) || /^#admin\b/.test(location.hash);
try { if (localStorage.getItem(CLE_ADMIN) === "1") ADMIN = true; } catch (e) {}
addEventListener("keydown", e => {
  if (e.ctrlKey && e.altKey && (e.key === "a" || e.key === "A")){
    e.preventDefault();
    try { localStorage.setItem(CLE_ADMIN, ADMIN ? "0" : "1"); } catch (err) {}
    location.reload();
  }
});

const enregistreConf = () => { try { localStorage.setItem(CLE_CONF, JSON.stringify(CONF)); } catch (e) {} };
const rgbHex = s => {
  const m = /rgb((d+),s*(d+),s*(d+))/.exec(s || "");
  return m ? "#" + [1, 2, 3].map(i => (+m[i]).toString(16).padStart(2, "0")).join("") : null;
};
const joli = id => id.replace(/^d+-/, "").replace(/[_-]+/g, " ").toLowerCase()
...
if (!ADMIN){
  ...
  $("btnLayers").onclick = () => {
    ...
    $("btnLayers").setAttribute("aria-pressed", o);
  };
  ...
  $("copieConf").onclick = e => {
    navigator.clipboard && navigator.clipboard.writeText(JSON.stringify({ apparence: CONF, dessins: DESSINS }, null, 2)).then(
...
function activeCalque(id){
  ...
  termineTrace(false);
  const c = trouveCalque(calqueActif);
  $("outils").classList.toggle("open", !!c);
  if (c) $("outilsCalque").textContent = "Dessin · " + c.nom;
...
function majElement(){
  ...
  const estTexte = f.t === "texte";
  $("elemTexte").hidden = !estTexte;
  $("elemTaille").hidden = !estTexte;
  if (estTexte){
...
function construitPanneau(){
  ...
  liste.forEach((x, i) => {
    ...
    if (x.t === "dessin"){
      nom.title = "Cliquez pour dessiner dessus, double-cliquez pour renommer";
      const cr = d.querySelector(".ren");
      ...
      nom.ondblclick = ev => { ev.preventDefault(); renommeCalque(x.ref); };
...
/* ============================================================
   ...
   ============================================================ */
function ouvreModale(titre, remplitCorps, boutons){
...
function fermeModale(){ $("modale").classList.remove("open"); }
...
function remplitOrdre(hote){
  ...
  pile().slice().reverse().forEach(x => {
    ...
    li.innerHTML = '<span class="poignee" aria-hidden="true">&#8942;&#8942;</span>' +
      '<span class="nom"></span>' +
      '<span class="bouts"><button data-h="1" title="Mettre au premier plan">&#8607;</button>' +
    ...
    li.querySelectorAll(".bouts button").forEach(b =>
